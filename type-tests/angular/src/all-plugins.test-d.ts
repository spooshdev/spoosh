...
const { injectRead, injectWrite, invalidate, prefetch, runGc } = create(spoosh);

// All options available together on injectRead
...
// All result fields available (as signals)
expectType<boolean>(read.meta().isOptimistic);
expectType<boolean>(read.meta().isInitialData);
const progress = read.meta().progress;
if (progress) {
  expectType<number>(progress.loaded);
}

// All trigger options available on injectWrite
