...
Every plugin can define up to 9 type interfaces:
...
For write operations, options are split into two categories:

- **Hook-level options** (`writeOptions`): Passed to `injectWrite` as the second argument. These options are set once when the inject is created and enable proper TypeScript inference.

- **Trigger-level options** (`writeTriggerOptions`): Passed to `trigger()` when executing the mutation. These options can vary per-call.
