...
```typescript
uploadFile = injectWrite((api) => api("files").POST(), { progress: true });

uploadFile.trigger({ body: form({ file: selectedFile }) });

// uploadFile.meta().progress?.loaded tracks bytes uploaded
```
