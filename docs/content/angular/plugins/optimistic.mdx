...
Use the `optimistic` function directly for external events like WebSocket messages:
...
> **Note:** Standalone mode only supports immediate updates (`filter` and `set`). Methods like `confirmed()`, `disableRollback()`, and `onError()` are not available since there's no mutation lifecycle.
