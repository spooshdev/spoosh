...
- **Hook-level options** (`writeOptions`): Passed to `useWrite` as the second argument. These options are set once when the hook is created and enable proper TypeScript inference.
